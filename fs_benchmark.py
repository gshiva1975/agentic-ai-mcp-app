...
Usage (from repo root, with venv active):
    python finance_service_benchmark.py --url http://localhost:8080
...
log = logging.getLogger("finance_service_benchmark")
...
def _load_llm():
    ...
    with step("LLM load"):
        from finance_service.llm import LocalLlamaLLM
        llm = LocalLlamaLLM()
    log.info("LLM ready")
    return llm
...
def _load_rag_components():
    ...
    with step("VectorStore init"):
        from finance_service.core.vector_store import VectorStore
        store = VectorStore()

    with step("EmbeddingModel init"):
        from finance_service.core.embedding_model import EmbeddingModel
        embed = EmbeddingModel()

    with step("ResearcherAgent init"):
        from finance_service.agents.researcher import ResearcherAgent
...
def run_optimized(llm, store, embed, researcher, query: str, evaluator) -> dict:
    qlog = logging.getLogger("finance_service_benchmark.optimized")
    qlog.info("OPTIMIZED  query='%s'", query[:80])

    with step("OptimizedFinancePipeline import"):
        from finance_service.optimized_pipeline import OptimizedFinancePipeline
        pipeline = OptimizedFinancePipeline(
...
def run_via_api(base_url: str, mode: str, query: str, evaluator) -> dict:
    alog = logging.getLogger("finance_service_benchmark.api")
...
def print_table(results: list):
    ...
    print(); print("═" * W)
    print("  FINANCE SERVICE BENCHMARK — Baseline (plain LLM) vs Optimized (RAG + Agentic)")
...
def main():
    parser = argparse.ArgumentParser(description="Finance Service hallucination benchmark")
    ...
    log.info("Finance Service Benchmark starting  url=%s  max=%s  debug=%s",
